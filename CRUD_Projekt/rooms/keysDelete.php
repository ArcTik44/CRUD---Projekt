...
use Tracy\Debugger;
Debugger::enable();
final class KeysDeletePage extends BaseDBPage{
    ...
    protected function setUp(): void
    {
        ...
        if ($this->state === self::STATE_PROCESSED) {
            ...
            if ($this->result === self::RESULT_SUCCESS) {
                ...
                $this->title = "Klíč byl úspěšně odebrán";
            ...
            $this->key = KeyModel::readPostData();
                ...
                $this->title = "Odebrat klíč : Neplatný formulář";
            }
    }
    ...
    public function body():string{
        ...
        $queryRooms = 'SELECT room.name AS roomName, room.room_id AS roomId FROM room';
        
        $stmtEmployees = DB::getConnection()->prepare($queryEmployees);
        $stmtRooms = DB::getConnection()->prepare($queryRooms);
        $stmtRooms->bindParam(':emp_id',$this->employee_id);

        $stmtEmployees->execute();
        $stmtRooms->execute();
        if ($this->state === self::STATE_FORM_REQUESTED) {
            return $this->m->render("keysDelete",['Employees'=>$stmtEmployees,"Rooms"=>$stmtRooms]);
        } elseif ($this->state === self::STATE_PROCESSED) {
            if ($this->result === self::RESULT_SUCCESS) {
                return $this->m->render("keysSuccess", ["message" => "Klíč byl úspěšně odebrán"]);
            } elseif ($this->result === self::RESULT_FAIL) {
                return $this->m->render("keysFail", ["message" => "Odebrání klíče selhalo"]);
            }
        }
        return "";
    }
